...
public class QueryPlan {
  ...
  private QueryOperator pushDownWheres(QueryOperator source, int except) throws QueryPlanException, DatabaseException {
    for (String column : this.whereColumnNames) {
      for (String sourceColumn : source.getOutputSchema().getFieldNames()) {
        if (source.checkColumnNameEquality(sourceColumn, column)) {
          int columnIndex = this.whereColumnNames.indexOf(column);
          if (columnIndex != except) {
            source = new WhereOperator(source, column, this.whereOperators.get(columnIndex), this.whereDataTypes.get(columnIndex));
          }
        }
      }
    }
  ...
  private QueryOperator minCostSingleAccess(String table) throws DatabaseException, QueryPlanException {
    ...
    int scanCost = this.transaction.getNumDataPages(table);
    ...
    int minWhereIdx = -1;
    int minWhereCost = scanCost;
    for (Integer i : whereIndices) {
      String column = this.whereColumnNames.get(i);
      PredicateOperator predicate = this.whereOperators.get(i);
      DataType value = this.whereDataTypes.get(i);
      IndexScanOperator scan = new IndexScanOperator(this.transaction, table, column, predicate, value);
      int cost = scan.estimateIOCost();
      if (cost < minWhereCost) {
        minWhereCost = cost;
        minWhereIdx = i;
        minOp = scan;
      }
  ...
  private QueryOperator minCostJoinType(QueryOperator leftOp,
    ...
    List<QueryOperator> choices = new ArrayList<>();

    choices.add(new SNLJOperator(leftOp, rightOp, leftColumn, rightColumn, this.transaction));
    choices.add(new PNLJOperator(leftOp, rightOp, leftColumn, rightColumn, this.transaction));
    choices.add(new BNLJOperator(leftOp, rightOp, leftColumn, rightColumn, this.transaction));
    choices.add(new GraceHashOperator(leftOp, rightOp, leftColumn, rightColumn, this.transaction));

    for (QueryOperator q : choices) {
      int estIO = q.estimateIOCost();
      if (estIO < minCost) {
        minCost = estIO;
        minOp = q;
      }
  ...
  private Map<Set, QueryOperator> minCostJoins(Map<Set, QueryOperator> prevMap,
    ...
    for (Map.Entry<Set, QueryOperator> leftEntry : prevMap.entrySet()) {
      List<String> leftColumns = leftEntry.getValue().getOutputSchema().getFieldNames();
      for (String leftColumn : leftColumns) {
        if (this.joinLeftColumnNames.contains(leftColumn)) {
          //need to go through every instance if there are multiple joins on that column
          int joinIndex = this.joinLeftColumnNames.indexOf(leftColumn);
          String rightTable = this.joinTableNames.get(joinIndex);
          String rightColumn = this.joinRightColumnNames.get(joinIndex);
          // check every pass 1 table set to see if it has rightTable in it
          // need to make sure right table isn't already in the set
          while (leftEntry.getKey().contains(rightTable) && joinIndex != this.joinLeftColumnNames.lastIndexOf(leftColumn)) {
            List<String> nextJoin = this.joinLeftColumnNames.subList(joinIndex + 1, this.joinLeftColumnNames.size());
            int newJoinIndex = nextJoin.indexOf(leftColumn) + joinIndex + 1;
            rightTable = this.joinTableNames.get(newJoinIndex);
            rightColumn = this.joinRightColumnNames.get(newJoinIndex);
            joinIndex = newJoinIndex;
          } // if the table is still in the set then there are no valid joins for this column
          if (leftEntry.getKey().contains(rightTable)) {
            continue;
          }
          Set<String> rightTableSet = new HashSet<>();
          rightTableSet.add(rightTable);
          QueryOperator rightOperator = pass1Map.get(rightTableSet);
          QueryOperator minOp = minCostJoinType(leftEntry.getValue(), rightOperator, leftColumn, rightColumn);
          rightTableSet.addAll(leftEntry.getKey());
          if (prevMap == pass1Map) {
            QueryOperator reverseMinOp = minCostJoinType(rightOperator, leftEntry.getValue(), rightColumn, leftColumn);
            int minOpCost = minOp.estimateIOCost();
            int reverseMinOpCost = reverseMinOp.estimateIOCost();
            if (reverseMinOpCost < minOpCost) {
              minOp = reverseMinOp;
            }
          }
          map.put(rightTableSet, minOp);
        }
      }
    }
    if (map.isEmpty()) {
      throw new QueryPlanException("Invalid Join");
    }
    return map;
  }

...
public class IntHistogram implements Histogram<Integer>{
  ...
  public float computeReductionFactor(PredicateOperator predicate,
    ...
    float numDistinct = getNumDistinct();
    float minValue = getMinValue();
    float maxValue = getMaxValue();
    float valueFloat = value.getInt();

    switch (predicate) {
      case EQUALS:
        reductionFactor = 1f / numDistinct;
        break;
      case LESS_THAN:
        reductionFactor = (valueFloat - minValue) / (maxValue - minValue);
        break;
      case LESS_THAN_EQUALS:
        reductionFactor = ((valueFloat - minValue) / (maxValue - minValue)) + (1f / numDistinct);
        break;
      case GREATER_THAN:
        reductionFactor = (maxValue - valueFloat) / (maxValue - minValue);
        break;
      case GREATER_THAN_EQUALS:
        reductionFactor = ((maxValue - valueFloat) / (maxValue - minValue)) + (1f / numDistinct);
        break;
      default:
        break;
    }
